...
public class SingleJedisUtil
{
    ...
    private static final SingleJedisUtil redis = new SingleJedisUtil();

    private static final Logger logger = LoggerFactory.getLogger(SingleJedisUtil.class);
...
//        this.OBJECT = new ObjectCls();
    ...
    static
    {
        ...
        ResourceBundle bundle = ResourceBundle.getBundle("dxa");
        if (bundle == null)
        {
        ...
        if (jedisPool == null)
        {
            ...
            config.setMaxWaitMillis(Long.valueOf(1000));
            config.setMaxTotal(Integer.valueOf(500));
            config.setMaxIdle(Integer.valueOf(200));
            config.setMinIdle(Integer.valueOf(100));
            config.setTimeBetweenEvictionRunsMillis(Long.valueOf(30000));
            config.setMinEvictableIdleTimeMillis(Long.valueOf(30000));
            config.setNumTestsPerEvictionRun(Integer.valueOf(5));
            config.setTestOnBorrow(Boolean.valueOf(false));
            config.setTestOnReturn(Boolean.valueOf(true));
            config.setTestWhileIdle(Boolean.valueOf(true));
            String ip = bundle.getString("rigol.redis.host");
            Integer port = Integer.valueOf(bundle.getString("rigol.reids.port"));
            int timeout = 2000;
            String password = bundle.getString("rigol.redis.password");
     ...
     * 获取SingleJedisUtil实例
     */
    public static SingleJedisUtil getInstance()
    {
        return redis;
    }
...
//                String[] ss = StringUtils.objectArrayToStringArray(value.toArray());
//                result = jedis.rpush(key, ss);
//
//                if (cacheSeconds != 0) {
...
//            returnJedis(sjedis);
//            return SerializeUtil.<T>reSerializableObject(value);
//        }
        ...
        public <T> T getObj(String key)
        {
            ...
            byte[] value = sjedis.get(SafeEncoder.encode(key));
            returnJedis(sjedis);
            return SerializeUtil.<T>reSerializableObject(value);
        }
        ...
        /**
         * 添加一条记录，带过期时间，仅当给定的key不存在时才插入
         *
         * @return 状态码，true插入成功且key不存在，false未插入，key存在
         */
        ...
        {
            Jedis jedis = getJedis();
            String status = jedis.set(SafeEncoder.encode(key), value);
            returnJedis(jedis);
            return status;
        }
        ...
        {
            Jedis jedis = getJedis();
            String status = jedis.set(SafeEncoder.encode(key), SerializeUtil.serialize(value));
            returnJedis(jedis);
            return status;
        }

        /**
         * 存储对象,添加记录,如果记录已存在将覆盖原有的value
         *
         * @return 状态码
         */
...
//        }
        ...
        public Set<String> getListToSet(String key) {
            Set<String> value = new HashSet<String>();
            Jedis jedis = null;
            try {
                jedis = getJedis();
        ...
        {
            Jedis jedis = getJedis();
            List<String> s = jedis.blpop(timeout,key);
            returnJedis(jedis);
            return s;
        }
